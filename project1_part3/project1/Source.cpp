	...
	try
	{
		...
		for (int i = 0; i < mazeWidth; i++)
		{
			for (int j = 0; j < mazeHeight; j++)
			{
				...
				if (adjList[i][j] > 0) {
					Vertex temp = Vertex(i, j, 0, 0, mazeData[i][j]);
...
extern "C" __declspec(dllexport) bool GetNextPosition(int& xpos, int& ypos) {
	...
	{
		times = 0;
		path = graph.AStar(graph, Vertex(startX, startY, 0, 0, adjList[startX][startY]), Vertex(endX, endY, 0, 0, adjList[endX][endY]), vertices, adjList);
		isSetStarted = true;
